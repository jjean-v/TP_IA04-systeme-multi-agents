...
| **Version document** | 1.1.4 |

## Préliminaires

Créer l’arborescence suivante à la racine de votre compte (`$HOME` ou `%USERPROFILE%`) :
...
Mettre la variable `PATH` à jour pour que tout fonctionne.
...
Écrire un programme qui affiche les nombres pairs entre 1 et 1000. Ne pas utiliser de condition. Pourquoi cette exigence ?
...
1. Écrire une fonction `Moyenne(sl []int) float64` qui prend en entrée un slice d’entiers et qui renvoie la moyenne de ses éléments.
2. Écrire une fonction `ValeursCentrales(sl []int) []int` qui prend en entrée un slice d’entiers et qui renvoie la ou les valeurs centrales (au sens statistique) du slice. Les valeurs centrales de `[1,5,3,2]` sont `2`et `3`. 
...
4. Écrire une fonction `Compte(sl []int)` qui affiche les nombres contenus dans le slice.
...
1. Écrire une structure `Point2D` et un constructeur `NewPoint2D(x, y float64) (*Point2D)` les getters/setters `(Point2D) X() float64`, `(*Point2D) SetX(x float64)`, `(Point2D) Y() float64`,  `(*Point2D) SetY(y float64)`, `(Point2D) Clone() (Point2D)`, `(Point2D) Module() float64` qui calcule sa distance à l'origine.
...
3. Écrire la structure `Sprite` qui contient une position, une *hitbox*, un facteur de zoom et le nom du fichier bitmap. Écrire un constructeur, des setters/getters, des méthodes `Move` qui déplace le sprite à une autre position et `Collision` qui calcule le rectangle collision entre 2 sprites.
