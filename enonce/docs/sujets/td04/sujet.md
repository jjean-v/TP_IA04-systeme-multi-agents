...
On pourra utiliser la fonction de départage suivante : le plus le numéro d'une alternative est petit, le plus elle est préférée.

### Quelques propriétés (final A21)

1. Montrer (directement, avec un contre-exemple) que le vote majoritaire à 2 tours ne respecte pas le critère de Condorcet.
...
2. Calculer le résultat de la méthode de Copeland.
...
4. Est-il possible de trouver une manipulation ?

## Exercice 4 : représentation des préférences des agents (final A21)

### Relations et scores

Soit $`\succsim`$ une relation transitive, réflexive et totale sur un ensemble fini $`E`$. On admet qu'une telle relation peut être *représentée* par une fonction de valeur $`V_\succsim: E \rightarrow \mathbb{N}`$ de manière à ce que $`a \succsim b \iff V_\succsim (a) \ge V_\succsim (b)`$.
...
Soit $`≿_1`$ et $`≿_2`$ deux relations totales, réflexives et transitives sur $`E`$.
...
4. La relation $`\mathcal{R}`$ est totale.
