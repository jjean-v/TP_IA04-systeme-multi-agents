...
## II. Implémentation des méthodes de vote  : création du package `comsoc`
...
- Les profils de préférences sont tels que si `profile` est un profil, `profile[12]` représentera les préférences du votant `12`. Les alternatives sont classées de la préférée à la moins préférée :  `profile[12][0]` represente l'alternative préférée du votant `12`.
...
On distingue les fonctions de bien-être social (*social welfare function*, *SWF*), qui retournent un décompte à partir d'un profil, des fonctions de choix social (*social choice function*, SCF) qui renvoient quant à elles uniquement les alternatives préférées.
...
Chaque fonction peut renvoyer une erreur, par exemple dans le cas de profils malformés.

On utilisera **un fichier source différent** par méthode de vote.
...
En cas d'égalité, les SCF doivent renvoyer un seul élément et les SWF doivent renvoyer un ordre total sans égalité. On utilisera pour cela des fonctions de *tie-break* qui étant donné un ensemble d'alternatives, renvoient la meilleure. Elles respectent la signature suivante (une erreur pouvant se produire si le slice d'alternatives est vide) :
